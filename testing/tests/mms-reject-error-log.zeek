...
event zeek_init() {
    ...
    local reject_pdu: RejectPDU = [
        $originalInvokeID=1902,
        $rejectReason=[$confirmed_errorPDU=RejectPDU_rejectReason_confirmed_errorPDU_invalid_invokeID]
    ];

    event RejectPDU_evt(c, "resp_to_orig", reject_pdu);
}
