...
3. 作为行为分析系统，我希望 MMS 业务日志预留 `src_mac` 和 `dst_mac`，从而后续可以由二层观测或通信关系模块补齐。
...
30. 作为通信关系模块，我希望 MMS 解析器不判断基线事实真假，从而基线策略仍由通信关系设计负责。
...
- 预留 MAC 和基线事实字段，但不让 MMS 解析层负责判断其真假。`src_mac`、`dst_mac`、`src_ip_seen`、`src_mac_seen`、`mms_ip_pair_seen` 和 `mms_full_pair_seen` 是面向富化的字段，判定策略属于通信关系层。
